...
private void HandleMageCatalogClient(ref Dictionary<string, Queue<Socket>> ipQueueDict, string ip)
{
    ...
    while (ipQueueDict[ip].Count > 0)
    {
        ...
        finally
        {
            ...
            if (!validated)
            {
                if (!FirewallManager.InspectIP(endpoint_string!, port: 8080))
                {
                    string banned = "Client was autobanned, dumping remaining connections.";
                    MessageWriter(msg_source, banned);
                    while (ipQueueDict[ip].Count > 0) 
                    { 
                        Socket dump = ipQueueDict[ip].Dequeue();
                        dump.Shutdown(SocketShutdown.Both);
                        dump.Close();
                        dump.Dispose();
                    }
                }
